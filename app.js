...
const ProductController = (function () {
  ...
  const data = {
    ...
    products: StorageController.getProducts(), // [] was for test before storage
  ...
  return {
    ...
    getProductById: function (id) {
      ...
      data.products.filter(function (prd) {
        if (prd.id == id) {
          product = prd;
        }
      });
...
})(); // IIFE (immediate function with private scope which can be opened as public)

// UI Controller
const UIController = (function () {
  const Selectors = {
    productList: "#item-list", // to change only once and here, not for any other places of the app
  ...
  return {
    ...
    addProductToForm: function () {
      ...
      console.log(selectedProduct);
      console.log(document.querySelector(Selectors.productName).value);
...
const AppController = (function (ProductCtrl, UICtrl, StorageCtrl) {
  ...
  const productAddSubmit = function (e) {
    ...
    if (productName !== "" && productPrice !== "") {
      // öç = like in counter strike :) , something new learned here to split the page
  ...
  const productEditClick = function (e) {
    if (e.target.classList.contains("edit-product")) {
      ...
      // set currenct product
      ProductCtrl.setCurrentProduct(product); // error here it took my half hour almost to see it :)
  ...
  return {
    init: function () {
      ...
      if (products.length == 0) {
        // error cause of "=", it was difficult to realize that. So later think about the solution
        // actually the problem is our console of the browser doesn't show any error to us, so that's why it is difficult to see it.
        UICtrl.hideCard();
      } else {
